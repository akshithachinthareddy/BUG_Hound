...
def login_auth():
    # Your secret key you got from Google reCAPTCHA
    ...
    # Check if reCAPTCHA returned a success
    if not result['success']:
        # If reCAPTCHA is failed, redirect back to the login page
...
def update_bug():
    ...
    resolution=['Pending', 'Fixed', 'Irreproducible', 'Deferred', 'As designed', 'Withdrawn by reporter', 'Need dre info', 'Disagree with suggestion', 'Duplicate']
...
def delete_bug(id_data):
    ...
    connection = pymysql.connect(**db_config)
    with connection.cursor() as cursor:
        # SQL query to soft delete a bug by setting the is_deleted flag and the deleted_at timestamp
        sql = "UPDATE bug SET is_deleted = %s, deleted_at = NOW() WHERE bug_id = %s"
        
        cursor.execute(sql, (True, id_data))

        
        connection.commit()
        message = f"Bug with id {id_data} was successfully marked as deleted"
    
    flash(message=message)
    return redirect(url_for('update_bug'))  # Assuming you want to redirect to the update_bug view after deletion

@app.route('/recover_all_bugs', methods=['GET','POST'])
def recover_all_bugs():
    ...
    connection = pymysql.connect(**db_config)
    with connection.cursor() as cursor:
        # Perform the query to recover the bug
        sql = "UPDATE bug SET is_deleted = 0, deleted_at = NULL WHERE is_deleted = 1 "
        cursor.execute(sql)
        connection.commit()
        flash('Bug recovered successfully!', 'success')

    return render_template('recover_bugs.html')
...
def search_bug():
    ...
    if request.method == 'POST':
        ...
        sql = "SELECT * FROM bug WHERE is_deleted = FALSE"
        conditions = []
